...
class AmavisPolicy
{
  ...
  public $policy_settings_default = array();
  public $policy_access = array();
  ...
  private static $policy_access_map = array(
    'virus_lover' => 'allow_virus_settings',
    'bypass_virus_checks' => 'allow_virus_settings',
    'virus_quarantine_to' => array('allow_virus_settings','allow_virus_setting_destination'),

    'spam_lover' => 'allow_spam_settings',
    'bypass_spam_checks' => 'allow_spam_settings',
    'spam_quarantine_to' => array('allow_spam_settings','allow_spam_settings_destination'),

    'banned_files_lover' => 'allow_banned_files_settings',
    'bypass_banned_checks' => 'allow_banned_files_settings',
    'banned_quarantine_to' => array('allow_banned_files_settings','allow_banned_files_settings_destination'),

    'bad_header_lover' => 'allow_bad_header_settings',
    'bypass_header_checks' => 'allow_bad_header_settings',
    'bad_header_quarantine_to' => array('allow_bad_header_settings','allow_bad_header_settings_destination')
    );
  
  function __construct( $db_config, $default_policy, $default_policy_access ) {
    ...
    $this->policy_settings_default = $default_policy;
    $this->policy_settings = $default_policy;
    $this->policy_access = $default_policy_access;
  ...
  // check policy access, return true as default
  function get_policy_access( $policy_access_list ) {
    $retval = true;
    // wrap non-array values in array
    if ( !is_array( $policy_access_list ) ) $policy_access_list = array($policy_access_list);
    foreach ( $policy_access_list as $policy_access_name ) {
      // deny policy if any mapped item is false
      if ( array_key_exists( $policy_access_name, $this->policy_access ) && $this->policy_access[$policy_access_name] === false ) {
        $retval = false;
        break;
      }
    }
  ...
  function apply_policy_access_map( $key, $value ) {
    // only process key which maps to policy access
    if ( array_key_exists( $key, self::$policy_access_map ) && $this->get_policy_access( self::$policy_access_map[$key] ) === false )
      return $this->policy_settings_default[$key];
    else
      return $value;
  }
  ...
  function map_to_db( $key, $value ) {
    ...
    // use defauly value if policy prevents user-specific settings
    $value = $this->apply_policy_access_map( $key, $value );
  ...
  function map_from_db( $key, $value ) {
    ...
    // use defauly value if policy prevents user-specific settings
    $retval = $this->apply_policy_access_map( $key, $retval );
